...
{
    string::iterator iter = str.begin();
    vector<pair<string, bool>> left;//规则
    string right;//原封不动存储方法体
    ...
    {
        string t;
        if(*iter == '%' && iter+1 != str.end())//处理自定义转义符号%之后的字符
        {
            iter++;
            switch(*iter)
            {
    ...
    while(iter != str.end() && *iter != ' ' && *iter != '\t')//去掉多余空格
        iter++;
    ...
    right += *iter++;//包括'{'
    int cnt = 1;
    while(cnt != 0 && iter != str.end())//读取方法体，并验证'{','}'是否匹配
    {
        if(*iter == '{')
            cnt++;
        else if(*iter == '}')
...
void Lex::addNFAEdge(int fromId, string input, int toId)
{
    nfaVet[fromId].push_back(Edge(fromId, input, toId));
}

void Lex::addDFAEdge(int fromId, string input, int toId)
{
    dfaVet[fromId].push_back(Edge(fromId, input, toId));
}
...
void Lex::buildNFA()
{
    ...
    for(auto iter = regrVet.begin(); iter != regrVet.end(); iter++)
    {
        int iStartId = itemId++;
        int iEndId = itemId++;
        addNFAEdge(startId, "", iStartId);
        addNFAEdge(iEndId, "", endId);//把多个NFA的首尾连起来构成一个更大的NFA
...
void Lex::findClosure(int i, string input, vector<int>* state)
{
    for(auto x : nfaVet[i])//类似java foreach遍历
    {
        int sId = x.toId;
        bool flag = true;
//        cerr<<x.fromId<<" "<<x.input<<" "<<x.toId<<endl;
        for(auto iter=state->begin(); iter!=state->end(); iter++)
            if((*iter) == sId)  //避免加入已经加入的状态
            {
//                cerr<<i<<"::"<<sId<<endl;
                flag = false;
                break;
        ...
        {
            state->push_back(sId);
            findClosure(sId, "", state);//找新加入状态的e闭包
        }
    }
//    for(auto iter=state->begin(); iter!=state->end(); iter++)
//        cerr<<*iter<<"<";
...
vector<int>* Lex::createDFAStatus(vector<int>* fromState, string input)
{
...
void Lex::NFA2DFA()
{
    ...
    t->push_back(0);
    //确定起始状态集合,0状态的e闭包
    vector<int>* startState =  createDFAStatus(t, "");
    delete t;
    ...
    while(!stateQue.empty())
    {
        ...
        for(string x : inputSet)//对当前已有的DFA状态的每种可能的输入
        {
            vector<int>* newState = createDFAStatus(state.first, x);
            if(newState->empty())
                continue;
            
            auto rel = stateMap.find(*newState);
            
            if(rel == stateMap.end())//如果生成新的状态
            {
                stateMap.insert(pair<vector<int>, int>(*newState, id));
                stateQue.push(pair<vector<int>*, int>(newState, id));
                addDFAEdge(state.second, x, id);
                id++;
            }
            else//状态已存在，直接添加相应的边
            {
                addDFAEdge(state.second, x, rel->second);
            }
        }
    }
    ...
    this->dfaNum = id;
    //获取dfa终态集合
    createDFAEndSet(stateMap);
}

void Lex::createDFAEndSet(map<vector<int>, int> stateMap)
{
